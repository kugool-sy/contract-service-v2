...
let matic_url = "https://api.polygonscan.com/api?module=stats&action=maticprice&apikey=DP59DP1UHEJ4Q4XX4R82M4ZX6KBZ9M676R"
...
async function getMaticPrice(url:string) {
    try{
        const response = await axios.get(url)
        return response
      }catch(e:any){
        console.log("get matic price error: \n", e)
      }
}
// run(url_mainnet);
...
//   await getFeeDataFromGastracker('maticmum')
    const price = await getMaticPrice(matic_url)
