...
export  async function getPermitSignature(signingKey, spender, value, nonce, deadline){

    const owner = computeAddress(signingKey.publicKey)
    const digest = getDigest(USDC_ON_polygon, owner ,spender, value, nonce, deadline)
