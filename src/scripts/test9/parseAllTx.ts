...
let onBoardResults:any[] = [];
let onBoardTxInfo:any[]= [];

const input_file = "0.csv"
const output_file = "out0.csv"
...
async function main() {
    ...
    for(const item of data) {
      if((item.Method === 'Remote Miner Onboard' || item.Method === 'Game Miner Onboard') && item.Status === '') {
        onBoardResults.push(item);
      }
    }
    console.log('total hash:', onBoardResults.length);


    for(let i=0; i<onBoardResults.length; i++){
    // for(let i=0; i<20; i++){

        console.log("count: ", i)

        try{
            const response = await provider.getTransaction(onBoardResults[i].Txhash)
            // console.log(receipt)
            ...
            const txInfo = abi.parseTransaction(response!)
            console.log(txInfo)

            if(txInfo?.name === 'GameMinerOnboard'){
                const record = {
                    owner: txInfo.args[0],
                    miner: txInfo.args[1],
                    method: "GameMinerOnboard",
                    tokenAddr:"",
                    tokenCount:0,
                    tx:onBoardResults[i].Txhash
                }

                onBoardTxInfo.push(record)
            }

            if(txInfo?.name === 'RemoteMinerOnboard'){
                const record = {
                    owner: txInfo.args[0],
                    miner: txInfo.args[1],
                    method: "RemoteMinerOnboard",
                    tokenAddr:txInfo.args[3][0],
                    tokenCount:BigInt(txInfo.args[3][1]).toString(),
                    tx:onBoardResults[i].Txhash
                }
            ...
            if( i % 10  === 0 ){
                try{
                    await csvWriter.writeRecords(onBoardTxInfo)
                    console.log("write file")
                    onBoardTxInfo = []
                    continue
                  }catch(e:any){
                    console.log("write error at number: ", i)
                    console.log(e)
                    return
                  }            
            }
    ...
    try{
        await csvWriter.writeRecords(onBoardTxInfo)
        console.log("write file last")
        console.log("complete")
        return
      }catch(e:any){
        // console.log("error at number: ", i)
        console.log(e)
        return
      }   
